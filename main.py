...
class Sys:
        ...
        if (turn == "player"):
            if len(self.cardSet["mySetCard"]) > 0:
                ...
                if len(self.cardSet["myHandCard"]) <= 6:
                    temp = self.cardSet["mySetCard"][sys.myCardOrder.pop(0)]
                self.cardSet["myHandCard"].append(Card(temp[0], temp[1], temp[2],
                                                       pygame.image.load(shared.path + "image/cardBack.png") if temp[
                                                                                                                    3] == None else pygame.image.load(
    ...
    def draw(self):
        ...
        if (self.placingCard and sys.isPlayerTurn):
            pygame.draw.rect(shared.screen, (255, 0, 0),
                             [shared.WIDTH / 2 - (shared.WIDTH / 80 * 8 + cardDim[0] * 7) / 2, shared.HEIGHT * 0.55,
                              (shared.WIDTH / 80 * 8 + cardDim[0] * 7), cardDim[1]], width=int(shared.WIDTH / 100))
            self.pointing = [0, 0]
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if (sys.checking):
                    if (0 <= mouse_pos[1] <= shared.HEIGHT * 0.3 or shared.HEIGHT * 0.7 <= mouse_pos[
                        1] <= shared.HEIGHT):
                        sys.checking = False
                elif (sys.isPlayerTurn and not (sys.placingCard or sys.checking) and len(
                        sys.cardSet["myCard"]) < 7 and shared.WIDTH * 0.8 <= mouse_pos[
                          0] <= shared.WIDTH and shared.HEIGHT * 0.7 <= mouse_pos[1] <= shared.HEIGHT * 0.9):
                    sys.ckeckingf()
